...
const storageClient = () =>
  (supabaseAdmin || supabase).storage.from("academic-pdfs");
const {
  generateLessonsFromText,
  deepExplainLesson,
} = require("../services/lessonAI.service");
...
async function extractFullText(pdfBuffer, userId) {
  ...
  console.log(
    `[Lessons] Extraction complete: ${result.fullText.length} chars, ${result.pageCount} pages${result.usedOCR ? " (via Gemini OCR)" : ""}`,
  );
  return {
...
async function runGenerationPipeline(pdfId, userId) {
  ...
  const { data: pdfBlob, error: dlErr } = await storageClient().download(
    `pdfs/${userId}/${pdfId}`,
  );

  if (dlErr) {
    throw Object.assign(new Error("PDF not found in storage"), {
      statusCode: 404,
  ...
  const lessonData = await generateLessonsFromText(
    extraction.fullText,
    pdfId,
    userId,
  );
  ...
  console.log(
    `[Lessons] ✅ Done — ${lessonData.totalLessons} lessons generated\n`,
  );
...
async function generateLessonsEndpoint(req, res) {
  ...
  try {
    ...
    if (inFlightGenerations.has(inflightKey)) {
      console.log(
        "[Lessons] Generation already in progress, returning queued status",
      );
      return res.status(202).json({
        success: true,
        status: "generating",
        message:
          "Lesson generation is already in progress. Poll GET /lessons/:pdfId/status for updates.",
      });
    }

    // 3. Start generation and register in dedup map
    const generationPromise = runGenerationPipeline(pdfId, userId).finally(
      () => {
  ...
  try {
    ...
    if (!row) {
      return res.status(404).json({
        ...
        message:
          "No lessons generated yet for this PDF. Call POST /lessons/generate first.",
      });
    }
...
async function deleteLessonsEndpoint(req, res) {
  ...
  try {
    ...
    return res
      ...
      .json({
        success: true,
        message: "Lessons cleared. Re-generate with POST /lessons/generate.",
      });
  } catch (err) {
    return res
      .status(500)
...
async function deepExplainEndpoint(req, res) {
  ...
  try {
    const result = await deepExplainLesson({
      title,
      explanation,
      key_points,
      documentTitle,
